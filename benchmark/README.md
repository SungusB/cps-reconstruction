...
| `spill-slot/` | Kotlin reuses one continuation spill field (`L$n`) for different variables at different suspension points (disjoint scopes: `run{}` blocks, sibling `if`s, sequential loops) and the single `$result` local for every suspension's value — a tainted and an untainted variable share a field, and the sink reloads from it | `NO-FLOW` (`04` is the `FLOW` positive control). Slot layout verified against `javap` per file. `01` caught a real false positive in the now-retired straight-line splice path — see its header. **FlowDroid (external baseline, `scripts/run_baseline.sh`) reports the false positive on `01`–`03` on raw bytecode and not on reconstructed bytecode; `04` is found either way** |
